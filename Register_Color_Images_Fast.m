function [successfulImages, validImageIndices, refIdx, tformList] = Register_Color_Images_Fast(colorImages, numImages, scene)
    close all; % Close all figure windows
    warning('off', 'vision:ransac:maxTrialsReached'); % disable warnings that come from RANSAC
    warning('off', 'MATLAB:nearlySingularMatrix'); % disable warnings that come from RANSAC
    ...
    for i = 1:numImages
        if i == refIdx
            ...
            validImageIndices = [validImageIndices, 1]; % Add reference to valid list
        ...
        if isempty(registeredColor) % If registration failed
            validImageIndices = [validImageIndices, 0]; % Store 0 for failure
        ...
        if validContentPercent < minValidContentThreshold
            ...
            validImageIndices = [validImageIndices, 0]; % mark as unsuccsesfull
            continue; % Skip this image
        end
    
        % Store the registered color image
        registeredColorImages{i} = registeredColor; % Store successful registration
        validImageIndices = [validImageIndices, 1]; % Add to valid list
    end

    % Create compact array with only successful images
    successfulImages = {};
    for i = 1:length(validImageIndices)
        if validImageIndices(i) == 1
            successfulImages{end+1} = registeredColorImages{i};
        end
    end

    % Apply common content mask to all registered images
    for i = 1:length(successfulImages)
        successfulImages{i} = im2double(successfulImages{i});
    end
    successfulImages = applyCommonContentMask(successfulImages);
    
    successfulImages = matchBrightnessAcrossImages(successfulImages);
end

%% -- Apply mask to keep only common content --
function maskedImages = applyCommonContentMask(registeredImages)
    if isempty(registeredImages) || numel(registeredImages) < 2
    ...
    % For each registered image, find valid content mask
    for i = 1:numel(registeredImages)
...
function [success, featureCount] = testRegistration(gray1, gray2, scene)
    ...
    try
        ...
        if featureCount >= 10 % If enough good matches found
...
function [registeredColor, tform] = registerImages(gray1, gray2, colorImg2, scene)
    ...
    try
        ...
        MaxDistance_list = [1.5, 5, 10, 50]; % List of distance thresholds to try
        ...
        if strcmp(scene, 'city')
            min_inliers = 5;
        else
            min_inliers = 7;
        end

        if inlierCount < min_inliers % If too few inliers
            registeredColor = []; % Return empty result
            return;
        end
