...
%   Creates a stacked area chart showing class proportions over time.
    ...
    dates = strings(1, numImages);  % Preallocate datetime array
    for i = 1:numImages
        tokens = regexp(imageFiles{i}, '(19|20)\d{2}', 'match');
        if isempty(tokens)
    ...
    x = cellfun(@str2double, dates);
    h = area(x, classPercents);  % Stacked area plot
